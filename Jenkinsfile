pipeline {
  ...
  environment {
    ...
    EXT_REPO = 'nntp2nntp'
    BUILD_VERSION_ARG = 'NNTP2NNTP_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-nntp2nntp'
    CONTAINER_NAME = 'nntp2nntp'
    DOCKERHUB_IMAGE = 'linuxserver/nntp2nntp'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/nntp2nntp'
    PR_DOCKERHUB_IMAGE = 'lspipepr/nntp2nntp'
    DIST_IMAGE = 'alpine'
    MULTIARCH = 'true'
    CI = 'true'
    CI_WEB = 'false'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/nntp2nntp") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/nntp2nntp",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-nntp2nntp",\
                       "backendUrl":"https://ghcr.io/linuxserver/nntp2nntp",\
                       "publicUrl":"https://lscr.io/linuxserver/nntp2nntp"}' || :
              else
